...
/**
 * Class NovalnetInstalmentSepaPaymentMethod
 *
 * @package Novalnet\Methods
 */
class NovalnetInstalmentSepaPaymentMethod extends PaymentMethodService
{
    /**
    ...
    {
       if ($this->configRepository->get('Novalnet.novalnet_instalment_sepa_payment_active') == 'true') {
        
        $active_payment_allowed_country = 'true';
        if ($allowed_country = $this->configRepository->get('Novalnet.novalnet_instalment_sepa_allowed_country')) {
        ...
        $minimum_amount = trim($this->configRepository->get('Novalnet.novalnet_instalment_sepa_minimum_order_amount'));
        ...
        $maximum_amount = trim($this->configRepository->get('Novalnet.novalnet_instalment_sepa_maximum_order_amount'));
    ...
    {  
        $name = trim($this->configRepository->get('Novalnet.novalnet_instalment_sepa_payment_name'));
        return ($name ? $name : $this->paymentHelper->getTranslatedText('novalnet_instalment_sepa'));
    }
    ...
    {
        $logoUrl = $this->configRepository->get('Novalnet.novalnet_instalment_sepa_payment_logo');
        if($logoUrl == 'images/sepa.png'){
            /** @var Application $app */
            $app = pluginApp(Application::class);
            $logoUrl = $app->getUrlPath('novalnet') .'/images/sepa.png';
        } 
        return $logoUrl;
    }
    ...
    {
        $description = trim($this->configRepository->get('Novalnet.novalnet_instalment_sepa_description'));
